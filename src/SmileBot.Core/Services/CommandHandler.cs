...
namespace SmileBot.Core.Services
{
    public class CommandHandler : ISmileService
    {
        ...
        private Task<(bool Success, string Error, CommandInfo Info)> ExecuteCommandAsync(CommandContext context, string input, int argPos, IServiceProvider serviceProvider)
            => ExecuteCommand(context, input.Substring(argPos), serviceProvider);
